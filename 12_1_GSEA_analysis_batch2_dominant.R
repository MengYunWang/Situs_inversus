...
#########################################
genes_in_solved <- readLines("genes_in_solved_cases_control_genes_excluded_dominant.txt")  
# Remove empty lines and trim whitespace
genes_in_solved <- trimws(genes_in_solved)
genes_in_solved <- genes_in_solved[genes_in_solved != ""] %>%
  unique(.)

genes_in_unsolved <- readLines("genes_in_unsolved_cases_control_genes_excluded_dominant.txt")
# Remove empty lines and trim whitespace
genes_in_unsolved <- trimws(genes_in_unsolved)
genes_in_unsolved <- genes_in_unsolved[genes_in_unsolved != ""] %>%
  unique(.)

genes_in_controls <- readLines("genes_in_control_case_genes_excluded_dominant.txt")
# Remove empty lines and trim whitespace
...
gsea_analysis <- function(gene_list_converted) {
  ...
  if (condition=="solved_cases") {
    ...
    write.xlsx(gsea_clean, file = "Gene_set_enrichment_dominant.xlsx", sheetName = condition, rowNames=TRUE, colNames=TRUE)

  } else {
    wb <- loadWorkbook("Gene_set_enrichment_dominant.xlsx")
    ...
    saveWorkbook(wb, "Gene_set_enrichment_dominant.xlsx", overwrite=TRUE)
  }  
}
