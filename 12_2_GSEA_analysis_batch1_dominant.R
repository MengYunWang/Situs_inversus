...
setwd("P:/workspaces/lg-sit/working_data/MengYun/data/batch1/dominant_model")
...
#########################################
genes_in_solved <- readLines("solved_genes_control_gene_excluded.dominant.txt")  
# Remove empty lines and trim whitespace
genes_in_solved <- trimws(genes_in_solved)
genes_in_solved <- genes_in_solved[genes_in_solved != ""]

genes_in_unsolved <- readLines("unsolved_genes_control_gene_excluded.dominant.txt")
# Remove empty lines and trim whitespace
genes_in_unsolved <- trimws(genes_in_unsolved)
genes_in_unsolved <- genes_in_unsolved[genes_in_unsolved != ""]

genes_in_controls <- readLines("control_genes_case_gene_excluded.dominant.txt")
# Remove empty lines and trim whitespace
...
gsea_analysis <- function(gene_list_converted) {
  ...
  ggsave(paste0("Gene_set_enrichment_", condition ,"_batch1_dominant.png") , plot = gsea_plot, width = 8, height = 7, units = 'in', dpi = 300)
  
  if (condition=="solved_cases") {
    
    write.xlsx(gsea_clean, file = "Gene_set_enrichment_batch1_dominant.xlsx", sheetName = condition, rowNames=TRUE, colNames=TRUE)
  } else {
    wb <- loadWorkbook("Gene_set_enrichment_batch1_dominant.xlsx")
    ...
    saveWorkbook(wb, "Gene_set_enrichment_batch1_dominant.xlsx", overwrite=TRUE)
  }  
}
