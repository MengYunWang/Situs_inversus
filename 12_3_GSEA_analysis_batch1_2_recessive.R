...
setwd("P:/workspaces/lg-sit/working_data/MengYun/data/batch1_2/recessive")
...
#########################################
genes_in_solved <- readLines("solved_cases_control_genes_excluded.batch1_2_recessive.txt")  
# Remove empty lines and trim whitespace
genes_in_solved <- trimws(genes_in_solved)
genes_in_solved <- genes_in_solved[genes_in_solved != ""] %>%
  unique(.)

genes_in_unsolved <- readLines("unsolved_cases_control_genes_excluded.batch1_2_recessive.txt")
# Remove empty lines and trim whitespace
genes_in_unsolved <- trimws(genes_in_unsolved)
genes_in_unsolved <- genes_in_unsolved[genes_in_unsolved != ""] %>%
  unique(.)

genes_in_controls <- readLines("control_case_genes_excluded.batch1_2_recessive.txt")
# Remove empty lines and trim whitespace
genes_in_controls <- trimws(genes_in_controls)
genes_in_controls <- genes_in_controls[genes_in_controls != ""] %>%
  unique(.)

genes_in_left_hander <- readLines("left_handers_cases_control_genes_excluded.batch1_2_recessive.txt")
# Remove empty lines and trim whitespace
genes_in_left_hander <- trimws(genes_in_left_hander)
genes_in_left_hander <- genes_in_left_hander[genes_in_left_hander != ""] %>%
  unique(.)
...
left_hander <- gconvert(genes_in_left_hander, organism = "hsapiens")$target
...
gsea_analysis <- function(gene_list_converted) {
  ...
  ggsave(paste0("Gene_set_enrichment_", condition ,"_batch1_2_recessive.png"), plot = gsea_plot, width = 8, height = 7, units = 'in', dpi = 300)
  
  
  if (condition=="solved_cases") {
    
    write.xlsx(gsea_clean, file = "Gene_set_enrichment_batch1_2_recessive.xlsx", sheetName = condition, rowNames=TRUE, colNames=TRUE)

  } else {
    wb <- loadWorkbook("Gene_set_enrichment_batch1_2_recessive.xlsx")
    ...
    saveWorkbook(wb, "Gene_set_enrichment_batch1_2_recessive.xlsx", overwrite=TRUE)
  }  
}
