...
#2. excluding synonymous SNV, unknown and .	
 ...
 #3. excluding gnomad41_exome_AF > 0.01	
	...
	# excluding the gnome_AF>0.00005
...
#5. Exclude if DP<7 or GQ <20	
	awk -F'\t' 'NR == 1 || ($271 >= 20 && $270 >= 7)' $sampleID/$sampleID.filtered.wo_syn_na_unkn.AF.alpha_cadd.txt > $sampleID/$sampleID.filtered.wo_syn_na_unkn.AF.alpha_cadd.DP_GQ_dominant.txt
