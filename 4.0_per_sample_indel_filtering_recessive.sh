...
#2. excluding synonymous SNV, unknown and .	
 ...
 #3. excluding gnomad41_exome_AF > 0.01	
	...
	# excluding the gnomad41_gnome_AF>0.005
...
#5. Exclude if DP<10 or GQ <20	
	awk -F'\t' 'NR == 1 || ($271 >= 20 && $270 >= 10)' $sampleID/$sampleID.filtered.wo_syn_na_unkn.AF.alpha_cadd.txt > $sampleID/$sampleID.filtered.wo_syn_na_unkn.AF.alpha_cadd.DP_GQ.txt
