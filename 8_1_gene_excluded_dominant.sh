...
awk -F';' '{for(i=1;i<=NF;i++) print $i}' genes_in_control.txt | sort -u > genes_in_control_dominant.txt
dos2unix genes_in_control_dominant.txt
...
for sample in "${samples[@]}"; do
  ...
  # Extract the 12th column (adjust the field separator if needed)
  awk -F'\t' '{print $12}' "$file" | grep -v '^Gene\.ensGene$' | awk -F';' '{ for(i=1;i<=NF;i++) print $i }' | sort -u > $sample/genes_in_$sample.dominant.txt
  dos2unix $sample/genes_in_$sample.dominant.txt
  
  # Find the common genes between controls and cases
  comm -12 genes_in_control_dominant.txt $sample/genes_in_$sample.dominant.txt > $sample/genes_in_overlap_dominant.txt
  dos2unix $sample/genes_in_overlap_dominant.txt
  
  
  # Filter out the overlap genes in cases
  out=${sample}/snp_indel_combined.filtered.${sample}.dominant.wCADD20_control_genes_excluded.txt
  ...
  awk -F'\t' -v OFS='\t' -v overlap="$sample/genes_in_overlap_dominant.txt" '
    # load overlap-list
    FNR==NR { bad[$1]; next }
	{
      # split field12 by ;
  ...
  ' "$sample/genes_in_overlap_dominant.txt" "$file" \
  >> "$out"
  
  
  #(head -n 1 "$file"; grep -v -F -f $sample/genes_in_overlap_dominant.txt "$file" | tail -n +2) > $sample/snp_indel_combined.filtered.$sample.dominant.wCADD20_control_genes_excluded.txt
...
awk -F';' '{for(i=1;i<=NF;i++) print $i}' genes_in_case.txt | sort -u > genes_in_case_dominant.txt
dos2unix genes_in_case_dominant.txt
...
for sample in "${samples[@]}"; do
  ...
  # Find the common genes between controls and cases
  comm -12 genes_in_case_dominant.txt $sample/genes_in_$sample.dominant.txt > $sample/genes_in_overlap_dominant.txt
  dos2unix $sample/genes_in_overlap_dominant.txt
  
  # Filter out the overlap genes in cases
  out=$sample/snp_indel_combined.filtered.$sample.dominant.wCADD20_case_genes_excluded.txt
  ...
  #(head -n 1 "$file"; grep -v -F -f $sample/genes_in_overlap_dominant.txt "$file" | tail -n +2) > $sample/snp_indel_combined.filtered.$sample.dominant.wCADD20_case_genes_excluded.txt
...
awk -F';' '{for(i=1;i<=NF;i++) print $i}' genes_in_case_control_genes_excluded.txt | sort -u > genes_in_case_control_genes_excluded_dominant.txt
dos2unix genes_in_case_control_genes_excluded_dominant.txt
...
done | sort | uniq -c > genes_in_case_wfreq_control_genes_excluded_dominant.txt